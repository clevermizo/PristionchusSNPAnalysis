...
for(snp in 1:nrow(cdsdata)){
  ...
  if(snp == 1){
  write.table(cdsdata[snp,],file="cdsdata_annotated.tab.7",sep="\t",row.names=FALSE,col.names=FALSE,quote=FALSE)
    }else{
    write.table(cdsdata[snp,],file="cdsdata_annotated.tab.7",append=TRUE,sep="\t",row.names=FALSE,col.names=FALSE,quote=FALSE)
    }
}
